...
const BookingSummary = ({ totalPrice, selectedSeats }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <p className="text-2xl font-bold text-blue-600">{totalPrice?.toLocaleString('vi-VN')}đ</p>
      ...
      {isOpen && (
        <div className="mt-2">
          ...
          <p className="mt-1 text-sm text-gray-500">Mã ghế/giường: {selectedSeats.join(', ')}</p>
        </div>
      )}
    </div>
  );
